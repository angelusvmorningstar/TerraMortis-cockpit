...
const STANDING_INSTRUCTION =
  ...
  + 'A haven location records where someone sleeps, not how or which way they '
  + 'travelled on a given night: never infer travel direction or method from a haven.';
...
function renderCharacterIndex(characterIndex, collisions) {
  ...
  for (const c of characters) {
    // Show the legal name when a moniker is in use, so a lookup by legal name resolves
    // (downtime submissions key on the legal character_name, not the moniker).
    const aka = c.legalName && c.legalName !== c.sortName ? ` (${c.legalName})` : '';
    lines.push(`- ${c.displayName}${aka} — ${c.clan} · ${c.covenant} · ${c.bloodline} · ${c.courtTitle}`);
    ...
    const npcs = c.npcs ?? [];
    if (npcs.length) {
      lines.push(`  npcs: ${npcs.map((n) => `${n.name}${n.isCorrespondent ? ' (correspondent)' : ''}`).join('; ')}`);
    }
  }
